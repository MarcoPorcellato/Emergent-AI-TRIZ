"""Single-attempt corrective runner for the A0-R2 tokenizer contract bug."""
...
from .a0r2c1_authorization import (
    AUTHORIZATION_PATH,
...
RUN_ID = "a0r2c1-v1.0.0-f8027fd0-r1"


def _corrected_activations(**kwargs):
    return run_a0r2_activations(
        **kwargs,
        adapter_factory=SmolLM2C1MappingAdapter,
    )
...
def main(argv: list[str] | None = None) -> int:
    ...
    if parsed.run_id != RUN_ID:
        raise base_runner.A0R2RunnerError(f"corrective run-id must be {RUN_ID}")
    if parsed.stage not in {"all", "verify"}:
        raise base_runner.A0R2RunnerError("corrective runner permits only all or verify")

    root = Path(parsed.root).resolve()
    verify_a0r2c1_contract(root)
    ...
    try:
        ...
        base_runner.verify_a0r2_sealed_execution_authorization = verify_a0r2c1_authorization
