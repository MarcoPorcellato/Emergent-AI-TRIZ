...
def validate_sealed_target_key(records: Sequence[Mapping[str, Any]], targets: Sequence[Mapping[str, Any]]) -> dict[str, Any]:
    """Require exactly one key per record and 6/6/6/6 primary position balance."""
    expected = {record.get("record_id"): record for record in records if isinstance(record, Mapping)}
    if len(records) != 72 or len(expected) != 72 or None in expected:
        raise Exp001TargetKeyError("primary record inventory must contain 72 unique IDs")
    ...
    for suffix in ("transfer-blinded", "lexical-control", "transfer-exposed"):
        choices = Counter(choice for record_id, choice in resolved.items() if record_id.endswith(suffix))
    ...
    return {"status": "balanced", "records": 72, "positions_per_condition": 6}
