...
def preflight_stage(stage_id: str, *, root: str | Path = ROOT) -> dict[str, Any]:
    ...
    if stage_id == "EXP-002B":
        ...
        key = _load(key_path)
        question_ids = [record["question_id"] for record in _load(repository / "results/exp002/preexecution/question-bank.json")]
        validate_answer_key(key, question_ids, question_bank_sha256=key["question_bank_sha256"])
    else:
