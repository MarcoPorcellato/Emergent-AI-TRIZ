...
description: One-command visual index for the maintained Lab 00 through Lab 04 artifacts and their scientific boundaries.
status: canonical
...
The laboratory suite is the public local entrance to the maintained Latent TRIZ artifacts. It provides one page for navigating Lab 00 through Lab 04 while keeping each artifact's scientific status explicit.
...
Lab 01 is empirical model instrumentation but remains claim-ineligible. The current Lab 00, Lab 02, Lab 03, and Lab 04 fixtures are synthetic or process-only and must not be interpreted as evidence for or against the Latent TRIZ Hypothesis.
